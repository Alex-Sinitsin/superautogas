<div class="grid-item mb-[20px] p-5 shadow-md relative xl:w-[32%] md:w-[48%] w-full">
    ...
    <div class="post-card__body py-3 text-[15px]">
        <div class="badges-group mb-[10px]">
            @if($post->is_published)
                <span
                    class="text-xs inline-block py-1 px-2.5 leading-none text-center whitespace-nowrap align-baseline font-bold bg-green-600 text-white rounded">Опубликована</span>
            @else
                <span
                    class="text-xs inline-block py-1 px-2.5 leading-none text-center whitespace-nowrap align-baseline font-bold bg-gray-300 text-gray-700 rounded">Скрыта</span>
            @endif
        </div>
        {!! $post->content !!}
    </div>
