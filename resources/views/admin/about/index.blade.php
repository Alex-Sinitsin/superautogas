    ...
    <div class="content p-5 w-screen overflow-auto">
        <x-page.header>
            @if(\Illuminate\Support\Facades\Route::is('*about*'))
                <x-page.title title="О компании" icon="store">
                    <x-slot name="subtitle">{{ Breadcrumbs::render('admin.about') }}</x-slot>
                </x-page.title>
            @endif
                ...
                @if(\Illuminate\Support\Facades\Route::is('about*'))
                    <div class="page-item shadow-md">
                        <div class="pages-title flex flex-wrap items-center justify-between border-b p-4">
                            <div class="text-2xl font-bold">{{ $pages[1]->title }}</div>
                            <div class="buttons">
                                <a href="{{ route('about.edit', ['about' => $pages[1]->id]) }}"
                        ...
                        <div class="pages-body p-4">{!! $pages[1]->content !!}</div>
                    </div>
                @endif
            @endif
        </div>
